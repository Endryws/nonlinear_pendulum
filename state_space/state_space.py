...
def Pendulum(state_vector , actual_time):
    ...
    ddotphi = (((-(k*(d**2)/(2*I)))*state_vector[0]) - (zeta/I)*state_vector[1] + (((k*d)/(2*I))*delta_f) - 
               ((m*g*D*sin(state_vector[0]))/(2*I)) - ((2*mu)/(pi*I))*atan(q*state_vector[1]))
...
number_of_divisions_per_period = 100
